...
Since Kubernetes **v1.25**, the older **PodSecurityPolicy (PSP)** was removed and replaced with **Pod Security Admission (PSA)** — a built-in Admission Controller.

Its job is simple:

> Before storing a Pod in `etcd`, check whether it follows the security policy attached to its namespace.
...
Pod Security Admission
      │
Does this Pod satisfy the namespace policy?
      │
 ┌───────────────┐
 ▼               ▼
Yes             No
...
### Why label the namespace, not the Pod?

The policy is attached to the **namespace**, not to individual Pods, because the rule applies to everything created inside it — not just one Pod.

Think of a namespace as an office building. If the building has a rule — "visitors must wear helmets" — every person entering has to follow it, regardless of who they are. In the same way, once a namespace carries a security policy, **every Pod created inside it must comply.**

### The Three PSA Labels

A namespace can carry up to three separate PSA labels, each controlling a different response to a violation:

**1. `enforce`**
```text
pod-security.kubernetes.io/enforce=baseline
```
The strictest of the three. If a Pod violates this policy, it is **rejected outright** — it's never created. Think of this as a security guard stopping you at the gate.

**2. `audit`**
```text
pod-security.kubernetes.io/audit=restricted
```
The Pod is still created, but the violation gets recorded in the audit log. Nothing is blocked — think of it like a CCTV camera logging that a rule was broken, without stopping anyone.

**3. `warn`**
```text
pod-security.kubernetes.io/warn=restricted
```
The Pod is created, but `kubectl` shows a warning message to the user. This is useful for gradually raising security standards without breaking anything yet.

A namespace commonly combines these at different strictness levels — for example, actively `enforce`-ing a lighter policy while only `warn`-ing and `audit`-ing a stricter one, so teams get visibility into what would need to change before adopting it fully.

### The Three Security Standards

Kubernetes defines three built-in Pod Security Standards, from least to most restrictive:

| Level | Restrictions | Typical Use |
|---|---|---|
| **Privileged** | Almost none — allows privileged containers, host networking, host PID/IPC | System-level software (e.g. CNI plugins) |
| **Baseline** | Blocks clearly dangerous settings like `privileged: true` or `hostNetwork: true` | Most regular applications |
| **Restricted** | Strict — requires `runAsNonRoot: true`, `allowPrivilegeEscalation: false`, dropping all Linux capabilities | Production workloads |
...
In the lab below, the namespace enforces the **Baseline** standard — strict enough to block an obviously risky Pod, while still being a reasonable default for general application workloads.
...
## Bonus — Confirm That Authorization Was Never the Problem

Run:

```bash
kubectl auth can-i create pods -n k8s-lab-restricted
```
...
This confirms something important: you had full permission to create Pods in this namespace the entire time. Authorization was never the issue in Step 2 — the privileged Pod was blocked purely because the **Admission Controller** found the resource itself non-compliant, independent of who created it.

This is the core distinction to hold onto:

> **Authorization checks the user. Admission Control checks the resource.**
